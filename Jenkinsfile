    ...
    stages {
        ...
        stage('Deploy To Staging') {
            ...
            steps {
                script {
                    sh '''
                    sudo -u ubuntu -H sh -c "sed -i 's/$imagename_stage:latest/$imagename_stage:${BUILD_NUMBER}/g' kube-landing-page/staging-landing-page-deploy.yaml"
                '''
                ...
                script {
                    try {
                        sh '''
                            sudo -u ubuntu -H sh -c "kubectl delete $(kubectl get rs -n production | grep "0" | cut -d' ' -f 1) -n staging"
                        '''
                    }
                    catch(Exception e) {
        ...
        stage('Deploy To Production') {
            ...
            steps {
                script {
                    sh '''
                    sudo -u ubuntu -H sh -c "sed -i 's/$imagename_prod:latest/$imagename_prod:${BUILD_NUMBER}/g' kube-landing-page/production-landing-page-deploy.yaml"
                    ''' 
                ...
                script {
                    try {
                        sh '''
                            sudo -u ubuntu -H sh -c "kubectl delete $(sh'''sudo -u ubuntu -H sh -c "kubectl get rs -n production | grep "0" | cut -d' ' -f 1"''') -n production"
                        '''
                    }
                    catch(Exception e) {
                        echo ' No replica set found. '
